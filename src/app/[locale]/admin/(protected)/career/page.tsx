...
import { DeleteCareerButton } from './delete-career-button';
...
export default async function AdminCareerPage({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params;
  const entries = await prisma.career.findMany({ orderBy: { startDate: 'desc' } });
    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-4 mb-10">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Career</h1>
        </div>
        <Link
      ...
      {entries.length === 0 ? (
        <p className="text-gray-500 dark:text-[#888] text-sm">No career entries yet.</p>
      ) : (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-gray-500 dark:text-[#888] truncate">
                  {entry.company} · {formatDateRange(entry.startDate, entry.endDate, locale)}
                </p>
              </div>

              <div className="flex items-center gap-1 shrink-0">
                <Link
                  href={`/${locale}/admin/career/${entry.id}/edit`}
                ...
                <DeleteCareerButton id={entry.id} position={entry.positionEn} />
              </div>
            </li>
          ))}
        </ul>
